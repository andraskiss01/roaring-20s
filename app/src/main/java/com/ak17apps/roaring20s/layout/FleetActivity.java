...
public class FleetActivity extends VerticalListViewActivity {
    ...
    protected void loadEntryList(String sortBy) {
        ...
        for(Vehicle v : vehicleList){
            ...
            if(v.isOwned()){
                if(v.getArrival() != null){
                    ...
                    if(v.getType().equals(Type.VEHICLE_SHIP_SMALL) || v.getType().equals(Type.VEHICLE_SHIP_BIG)
                        ...
                        if(!driver.equals("-") && !stoker.equals("-")){
                            status = "Ready to go";
                        }else{
                            status = "Available";
                        }
            ...
            item.setTopRowItems(getIndicator(v.getType().substring(v.getType().indexOf("_") + 1).replace("_", " ")), getIndicator(v.getName()), getIndicator("Speed: " + v.getSpeed() + " m/h"), getIndicator("Cons.: " + v.getConsumption() + " " + consUnit), getIndicator("Load: " + v.getLoad() + "/" + v.getCapacity() + " gal")/*, getIndicator("Status: " + status)*/);
            item.setMiddleRowItems(getIndicator("Buy price: $" + v.getBuyPrice()), getIndicator("Sell price: $" + v.getSellPrice()), getIndicator("Status: " + status));

            if(v.getType().equals(Type.VEHICLE_CAR)){
                item.setBottomRowItems(sellBuyBtn, getIndicator("Arrival: " + (v.getArrival() != null ? v.getArrival() : "-")), goBtn);
            }else {
                item.setBottomRowItems(sellBuyBtn, getIndicator("Crew: " + (v.getType().equals(Type.VEHICLE_SHIP_SMALL) || v.getType().equals(Type.VEHICLE_SHIP_BIG) || v.getType().equals(Type.VEHICLE_TRAIN_SMALL) || v.getType().equals(Type.VEHICLE_TRAIN_BIG) ? driver + ", " + stoker : driver)), selectCrewBtn, getIndicator("Arrival: " + (v.getArrival() != null ? v.getArrival() : "-")), goBtn);
            }
